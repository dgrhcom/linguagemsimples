...
export function DocumentTypeSelector({
  ...
  const categories = [
    { id: "all", label: "Todos os Tipos", count: documentTypesData.length },
    { id: "normativo", label: "Atos Normativos", count: documentTypesData.filter(d => d.category === "normativo").length },
    { id: "correspondencia", label: "Correspondência", count: documentTypesData.filter(d => d.category === "correspondencia").length },
    { id: "administrativo", label: "Administrativo & Declarações", count: documentTypesData.filter(d => d.category === "administrativo").length }
  ];
    ...
    <div className="space-y-3">
      ...
      {previewingDoc && (
        <div className="fixed inset-0 z-60 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-2xl w-full shadow-2xl border border-zinc-200 animate-in zoom-in-95 space-y-4 max-h-[90vh] flex flex-col">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <span className="text-xs text-zinc-500 capitalize">
                    {previewingDoc.category} • Padrão Unicamp
                  </span>
                </div>
              </div>
              <button
                ...
                className="p-1.5 rounded-xl text-zinc-500 hover:text-black hover:bg-zinc-100 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-1">
              ...
              {/* Imagem do Gabarito */}
              <div className="relative aspect-[1/1.414] w-full border border-zinc-200 rounded-xl overflow-hidden bg-zinc-50 shadow-inner">
                {previewingDoc.modelImagePath ? (
                  <Image
                    src={previewingDoc.modelImagePath}
                    alt={`Gabarito de ${previewingDoc.label}`}
                    fill
              ...
              {previewingDoc.expectedSections && (
                <div className="bg-zinc-50 border border-zinc-200 rounded-2xl p-3.5 space-y-2 text-xs">
                  <span className="font-bold text-black block">Estrutura Esperada:</span>
                  <div className="flex flex-wrap gap-1.5">
                    {previewingDoc.expectedSections.map((sec, idx) => (
                      <span
                        key={idx}
                        className="bg-white border border-zinc-300 text-zinc-800 px-2 py-0.5 rounded-md text-[11px] font-medium"
                      >
                        {sec}
                      </span>
                    ))}
            ...
            <div className="flex items-center justify-between pt-2 border-t border-zinc-200">
              ...
              <button
                ...
                className="bg-[#FBB040] hover:bg-[#e59b2b] text-black font-black text-xs px-4 py-2 rounded-xl border border-[#d98a1a] shadow-xs flex items-center gap-1.5"
              >
      ...
      <div className="bg-[#faf9f5] border border-zinc-200 rounded-2xl p-3.5 sm:p-4 flex flex-wrap items-center justify-between gap-3 shadow-2xs">
        ...
        <div className="flex items-center gap-2">
          {selectedDocInfo.modelImagePath && (
            <button
              type="button"
              onClick={() => setPreviewingDoc(selectedDocInfo as DocumentTypeMetadata)}
      ...
      {/* Grade de Cards Expansível de Todos os Modelos */}
      {isOpen && (
            ...
            {filteredDocs.map(doc => {
              ...
              return (
                <div
                  ...
                  className={`relative text-left p-3 rounded-2xl border transition-all cursor-pointer flex flex-col justify-between group hover:shadow-md ${
                    isSelected
                      ? "bg-amber-50/60 border-[#FBB040] ring-2 ring-[#FBB040]/30 shadow-xs"
                      : "bg-white border-zinc-200 hover:border-zinc-300 hover:bg-zinc-50/70"
                  }`}
                >
                  <div className="space-y-2">
                    {/* Topo do Card: Miniatura e Categoria */}
                      ...
                      {isSelected ? (
                        ...
                        doc.modelImagePath && (
                          <button
                            ...
                            className="text-[10px] text-zinc-400 hover:text-black hover:bg-zinc-200 p-1 rounded-md transition-colors"

import Link from "next/link";
import { Sparkles, CheckCircle2, ShieldCheck, HelpCircle, ArrowRight, BookOpen, AlertCircle, Eye } from "lucide-react";

export default function ComoFuncionaPage() {
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-section space-y-section">
      <div className="text-center space-y-3">
        <h1 className="font-display text-heading sm:text-heading-lg text-ink">
          Como funciona a ferramenta?
        </h1>
        <p className="text-body text-charcoal max-w-2xl mx-auto leading-relaxed">
          Entenda como seu texto é analisado passo a passo e conheça o compromisso de transparência do nosso motor metodológico.
        </p>
      </div>

      {/* Passo a Passo */}
      <div className="space-y-6">
        <h2 className="text-subheading font-display text-ink flex items-center gap-2">
          <BookOpen className="w-5 h-5 text-amber" />
          <span>O Ciclo de Avaliação em 6 Passos</span>
        </h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-paper rounded-card border border-sand p-6 space-y-2">
            <span className="w-7 h-7 rounded-tile bg-ink text-amber font-bold text-xs flex items-center justify-center">1</span>
            <h3 className="font-semibold text-ink text-body-sm">Cole ou envie seu texto</h3>
            <p className="text-body-sm text-charcoal leading-relaxed">
              Insira seu comunicado, e-mail, ofício, relatório ou texto geral na área de edição.
            </p>
          </div>

          <div className="bg-paper rounded-card border border-sand p-6 space-y-2">
            <span className="w-7 h-7 rounded-tile bg-ink text-amber font-bold text-xs flex items-center justify-center">2</span>
            <h3 className="font-semibold text-ink text-body-sm">Informe quem vai ler (Opcional)</h3>
            <p className="text-body-sm text-charcoal leading-relaxed">
              Indicar o público-alvo e o objetivo ajuda a calibrar o nível de vocabulário e a avaliar se termos técnicos são adequados.
            </p>
          </div>

          <div className="bg-paper rounded-card border border-sand p-6 space-y-2">
            <span className="w-7 h-7 rounded-tile bg-ink text-amber font-bold text-xs flex items-center justify-center">3</span>
            <h3 className="font-semibold text-ink text-body-sm">A ferramenta analisa o conteúdo</h3>
            <p className="text-body-sm text-charcoal leading-relaxed">
              O motor híbrido cruza regras determinísticas da Unicamp e inteligência contextual para identificar frases longas, jargões, arcaísmos e formas excludentes.
            </p>
          </div>

          <div className="bg-paper rounded-card border border-sand p-6 space-y-2">
            <span className="w-7 h-7 rounded-tile bg-ink text-amber font-bold text-xs flex items-center justify-center">4</span>
            <h3 className="font-semibold text-ink text-body-sm">Você entende os problemas</h3>
            <p className="text-body-sm text-charcoal leading-relaxed">
              Cada apontamento explica <em>por que</em> o trecho precisa ser melhorado e cita a diretriz correspondente da Unicamp.
            </p>
          </div>

          <div className="bg-paper rounded-card border border-sand p-6 space-y-2">
            <span className="w-7 h-7 rounded-tile bg-ink text-amber font-bold text-xs flex items-center justify-center">5</span>
            <h3 className="font-semibold text-ink text-body-sm">Decida quais sugestões aceitar</h3>
            <p className="text-body-sm text-charcoal leading-relaxed">
              Você tem total controle para aceitar uma sugestão, aplicar em lote ou editar manualmente.
            </p>
          </div>

          <div className="bg-paper rounded-card border border-sand p-6 space-y-2">
            <span className="w-7 h-7 rounded-tile bg-ink text-amber font-bold text-xs flex items-center justify-center">6</span>
            <h3 className="font-semibold text-ink text-body-sm">Exporte o resultado</h3>
            <p className="text-body-sm text-charcoal leading-relaxed">
              Baixe a versão revisada ou o relatório completo em DOCX, PDF, Markdown, HTML ou TXT.
            </p>
          </div>
        </div>
      </div>

      {/* Transparência e Uso da IA */}
      <div className="bg-paper rounded-card border border-sand p-8 space-y-6">
        <h2 className="text-subheading font-display text-ink flex items-center gap-2">
          <Eye className="w-5 h-5 text-amber" />
          <span>Transparência: Como a Inteligência Artificial é Utilizada?</span>
        </h2>

        <div className="space-y-4 text-body-sm text-charcoal leading-relaxed">
          <p>
            A inteligência artificial funciona como uma <strong className="text-ink">assistente de apoio à redação</strong>, e não como um substituto do julgamento humano.
          </p>

          <div className="bg-sand/30 p-5 rounded-tile border border-sand space-y-2">
            <strong className="text-ink block font-semibold text-body-sm">1. Separação de Regras e IA:</strong>
            <p className="text-body-sm text-charcoal">
              Regras objetivas (contagem de palavras, frases &gt; 20 palavras, fórmulas obsoletas como &ldquo;DD./Ilmo.&rdquo;, grafia de horas e dicionários) são avaliadas por um motor determinístico rigoroso baseado na Unicamp. A IA é acionada apenas para interpretação contextual e reescrita fluida.
            </p>
          </div>

          <div className="bg-sand/30 p-5 rounded-tile border border-sand space-y-2">
            <strong className="text-ink block font-semibold text-body-sm">2. Preservação Semântica e Não Reducionismo:</strong>
            <p className="text-body-sm text-charcoal">
              A simplificação nunca deve omitir obrigações, prazos, condições jurídicas, valores ou referências normativas. Nosso pipeline valida semanticamente se nenhum dado essencial foi alterado.
            </p>
          </div>

          <div className="bg-sand/30 p-5 rounded-tile border border-sand space-y-2">
            <strong className="text-ink block font-semibold text-body-sm">3. Responsabilidade Final do Emissor:</strong>
            <p className="text-body-sm text-charcoal">
              Toda sugestão gerada pela ferramenta deve ser revisada por quem assina ou publica o documento antes da divulgação final.
            </p>
          </div>
        </div>
      </div>

      <div className="text-center pt-4">
        <Link
          href="/analisar"
          className="ghost-link text-body-sm font-semibold text-ink inline-flex items-center gap-2"
        >
          <span>Experimentar Agora no Analisador</span>
          <ArrowRight className="w-4 h-4" />
        </Link>
      </div>
    </div>
  );
}
